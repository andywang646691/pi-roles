...
skills: all                    # skill listing in the system prompt
...
| `tools` | no — see below | releases to **none** (deterministic, no leaking the previous role's toolset); chains that extend `pi` restore Pi's fresh-session default set |
| `skills` | no — see below | releases to **none**; chains that extend `pi` already carry all loaded skills |
| `intercom` | no | falls back to the global `intercomMode` setting |
| `extends` | no | role inherits from another role |

#### `tools` and `skills` — the shared collection directive

`tools` and `skills` share one syntax, one normalization, and one `extends`
merge pass (in src/roles.ts — they literally cannot drift apart). Four
spellings, four meanings:

| YAML | Meaning |
|---|---|---|
| `tools: read, bash` / `skills: git-review, jira` | **set** — exactly these |
| `tools:` / `skills:` (present, empty) | **disable entirely** (read-only session / no skill listing) |
| `all` | **everything in the domain** — every registered tool (incl. MCP) / every loaded skill |
| field absent | **inherit** — from parent role (`extends`); if no parent sets it, **none** (both fields) |

`all` is a reserved token: a tool or skill literally named `all` cannot be
addressed by name.

For `tools` there is one additional resolved state you never write by hand:
`default`, meaning **Pi's fresh-session baseline tool set**. It is what chains
that transitively include the built-in `pi` role get when they don't write
`tools:` themselves — the extension snapshots the pristine active-tool set at
`session_start` (before any role touches it), so `extends: pi` always restores
"what an unextended pi session would have", including your global config and
any MCP tools that auto-enable. The snapshot is taken per session, so it never
goes stale and never hardcodes Pi's defaults.

The deterministic absent→none default is deliberate: a plain custom role is
**self-contained**. It doesn't declare it, it doesn't get it — switching from a
tool-heavy role to a lean one cannot leak the previous role's toolset, and role
behavior never depends on switch history.

MCP tool refs work in the same field, mirroring `pi-subagents`:
...
#### `skills` — which skills the role lists

Skills are injected into the system prompt as a **progressive-discovery
listing** — name + description per skill, exactly the format Pi's default
prompt uses (generated by Pi's own `formatSkillsForPrompt`, so output is
byte-identical). The full SKILL.md is loaded by the agent on demand, as in a
default session.

- `skills: all` — every skill loaded for the current cwd (`.pi/skills/`,
  `~/.pi/agent/skills/`, `--skill`, settings, packages).
- `skills: git-review, jira` — exactly those; unknown names are skipped with
  a warning; skills with `disable-model-invocation: true` are skipped with a
  warning (they're only reachable via `/skill:name` in any pi session).
- `skills:` — no listing.
- Absent — inherit, else **none**.
- **`extends: pi` interacts specially**: the built-in pi role's live default
  prompt already contains every loaded skill, so a narrowing directive on a
  pi-derived chain is ignored with a warning (you cannot subtract from the
  default-prompt blob). `skills: all` on such a chain is a no-op, no warning.
  `tools` has no such restriction — tools are applied via `setActiveTools`, not
  the prompt blob, so `extends: pi` + `tools: read, bash` applies normally.
...
- `tools` / `skills`: same shared directive as standalone roles — **set overrides; empty disables; `all` takes everything; absent inherits**, and if no ancestor sets the field, both resolve to **none** (or, for chains through built-in `pi`, `tools` → the fresh-session baseline and `skills` → all, which the marker already carries).
...
| `warnOnMissingMcp` | `true` | Whether to surface warnings for unresolvable resources: a role's `mcp:*` entry for which no tool is registered, or a `skills:` name that isn't loaded this session. |
...
- **Role inheritance**: `model`/`thinking` override; `tools`/`skills` share one directive algebra (set/empty/`all`/absent — see above); markdown body is **prepended**.
