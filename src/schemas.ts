...
export const RoleFrontmatterSchema = Type.Object(
  {
    ...
    /**
     * Skill list as a raw, comma-separated string. Empty string means "no
     * skills". The token `all` means every skill loaded for the current cwd.
     *
     * Semantics mirror the `tools` field exactly (same schema shape, shared
     * normalization in roles.ts): absent → inherit, empty/null → none,
     * `all` → everything, otherwise exactly these skills.
     *
     * Skills render into the system prompt as a progressive-discovery
     * listing (name + description per skill) in the exact format Pi's own
     * default prompt uses — generated by Pi's `formatSkillsForPrompt`,
     * full SKILL.md loaded on demand by the agent.
     *
     * Interaction with `extends: pi`: the built-in pi role's live default
     * prompt already contains every loaded skill, so a narrowing directive
     * on a chain that includes pi is ignored with a warning at compose time
     * (see composeSkillsBlock in index.ts).
     */
    skills: Type.Optional(
      Type.Union([Type.String(), Type.Null()], {
        description:
          "Comma-separated skill names, or 'all' for every loaded skill. Empty/null = no skills.",
      }),
    ),
...
/**
 * Shared directive algebra for the `tools` and `skills` frontmatter fields.
 * Both fields normalize through the same function (`normalizeCollection` in
 * roles.ts), so the four YAML spellings map to identical kinds:
 *
 *   - absent            → `{ kind: "inherit" }`
 *   - `a, b`            → `{ kind: "set", names: ["a", "b"] }`
 *   - empty / `null`    → `{ kind: "set", names: [] }`  (explicit none)
 *   - `all`             → `{ kind: "all" }`
 *
 * `tools` additionally carries `{ kind: "default" }` — the one kind
 * `skills` never produces — meaning "Pi's fresh-session baseline tool set".
 * It is resolved at apply time against the snapshot captured on
 * `session_start`, and only emitted for resolve results whose `extends`
 * chain transitively includes the built-in `pi` role with no explicit
 * `tools:` directive.
 */
...
/** Tools directive. `all` = every registered tool; `default` = session-start baseline. */
export type ToolsDirective = CollectionDirective | { kind: "default" };

/** Skills directive. Skilla have no "default" kind — only tools do. */
export type SkillsDirective = CollectionDirective;
/**
...
export interface ResolvedRole {
  ...
  /** Final skills directive after merging the extends chain. */
  skills: SkillsDirective;
...
/**
 * Reserved token meaning "everything in this domain" for the `tools` and
 * `skills` frontmatter fields (normalized in roles.ts). A tool or skill
 * literally named `all` cannot be addressed by name — it is shadowed by
 * the token; documented in the README.
 */
export const ALL_TOKEN = "all" as const;
